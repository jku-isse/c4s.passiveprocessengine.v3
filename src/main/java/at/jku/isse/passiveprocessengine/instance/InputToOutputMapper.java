...
public class InputToOutputMapper {
	...
	protected static Set<Repair> getConcreteRepairs(Set<Object> objects, int limit,ConsistencyRuleType crd, Instance contextInstance) {
		if(childW == null)
			childW = getOrCreateWorkspaceByName("childWorkspace",crd.workspace,	WorkspaceService.ANY_USER, null, false, false);
		else {
			childW.update();
	...
	private static Workspace getOrCreateWorkspaceByName(String name, Workspace parentWorkspace, User user, Tool tool, boolean autoUpdate, boolean autoCommit) {
	    Optional<Workspace> workspace = WorkspaceService.allWorkspaces().stream().filter(w -> w.name().equals(name)).findAny();
	    if (workspace.isPresent()) {
	        return workspace.get();
	    } else
	    	return WorkspaceService.createWorkspace(name, parentWorkspace, user, tool, autoUpdate, autoCommit);
	}
