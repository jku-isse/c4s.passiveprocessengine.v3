...
class RepairAnalysisTests {
	...
	void testCollectionAndPropertyChange() throws ProcessException {
		...
		// this should invalidate the output as we now have a requirement that doesn;t fulfill output condition
		// hence we should have a negative impact of adding jiraB, (even though we need (req but not sufficient) it to fulfill the postcond eventually) on postcond
		// yet as we dont have any other constraints evaluated here, we wont see the effect of adding here just yet
		// here we only see the impact of setting jiraA to Open (from the initializatio of the jira item).
		ws.concludeTransaction();
		...
		// here we should see the adding of jiraB as positive as the repair of postcond should include adding something to 'requirements' of jiraA,
		// (as this repair suggestion is not there, the action is neutral/none)
		// closing jiraA should be positive towards postCond, and negative towards precond
		ws.concludeTransaction();
		repAnalyzer.printImpact();
		repAnalyzer.getImpact().clear();
		
		// now lets set the state of JiraB so that is fulfills the requirement, but also add another unfulfilling at the same time
		...
		TestArtifacts.setStateToJiraInstance(jiraA, JiraStates.Open); // to trigger a reeval of the precondition		
		ws.concludeTransaction();
		repAnalyzer.printImpact();
		repAnalyzer.getImpact().clear();
		...
		assert(proc.getProcessSteps().stream()
				.filter(step -> step.getDefinition().getName().equals("sd1") )
				.allMatch(step -> (step.getOutput("jiraOut").size() == 2) && step.getActualLifecycleState().equals(State.ENABLED) ));
	...
	public static ProcessDefinition getSingleStepProcessDefinition(Workspace ws) throws ProcessException {
		...
		QAConstraintSpec qa2 = QAConstraintSpec.createInstance("sd1-qa1-state", "self.out_jiraOut->size() > 0 and self.out_jiraOut->forAll( issue | issue.state = 'Closed')", "All linked issues should be closed", 2,ws);
